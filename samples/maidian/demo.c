...
int main(int args, char **argv)
{
    ...
    if (SA_OK != sa_init_logging_consumer("demo", &consumer))
    {
    ...
    // 在这个 Demo
    // 中，我们以一个典型的电商产品为例，描述一个用户从匿名访问网站，到下单购买商品，再到申请售后服务，
    // 这样一个整个环节，使用 Sensors Analytics（以下简称
    // SA）的产品，应该如何记录日志。

    // 特别需要注意的是，这个 Demo 只是描述 SA
    // 的数据记录能力，并不是说使用者要完全照搬这些 Event 和 Property
    // 的设计，使用者还是需要结合自己产品的实际需要，来做相应的设计和规划

    // 1. 用户匿名访问网站.
    const char *cookie_id =
        "ABCDEF123456789"; // 用户未登录时，可以使用产品自己生成的cookieId来标注用户.
    ...
    // 对于预置字段，已经确定好了字段类型和字段的显示名.
    {
        ...
        SA_ASSERT(SA_OK ==
                  sa_add_string("$os", "iOS", strlen("iOS"), properties));
        // 操作系统的具体版本.
        SA_ASSERT(SA_OK ==
                  sa_add_string(
                      "$os_version", "10.0.0", strlen("10.0.0"), properties));
        // 请求中能够拿到用户的IP，则把这个传递给SA，SA会自动根据这个解析省份、城市.
        SA_ASSERT(SA_OK == sa_add_string("$ip",
                                         "123.123.123.123",
                                         strlen("123.123.123.123"),
                                         properties));
        // 是否首次访问.
    ...
    // 1.2 记录用户首次登陆时间.
    {
        ...
        // 用户首次使用时间.
        SA_ASSERT(SA_OK ==
                  sa_add_date("first_time", time(NULL), 0, properties));
    ...
    // 1.3 搜索商品.
    {
        ...
        // 记录搜索商品事件.
        SA_ASSERT(SA_OK ==
                  sa_track(cookie_id, "SearchProduct", properties, sa));
    ...
    // 1.4 浏览商品.
    {
        ...
        // 商品名称.
        SA_ASSERT(SA_OK ==
                  sa_add_string("product_name", "XX手机", 8, properties));
        // 商品Tag.
        SA_ASSERT(SA_OK ==
                  sa_append_list("product_tag", "大屏", 6, properties));
        SA_ASSERT(SA_OK ==
                  sa_append_list("product_tag", "双卡双待", 12, properties));
        // 商品价格.
    ...
    // 2.1 通过，track_signup，把匿名ID和注册ID关联起来.
    {
        ...
        // 关联注册用户与匿名用户.
        SA_ASSERT(SA_OK ==
                  sa_track_signup(login_id, cookie_id, properties, sa));
    ...
    // 2.2 用户注册时，填充了一些个人信息，可以用Profile接口记录下来.
    {
        ...
        // 用户注册日期.
        SA_ASSERT(SA_OK ==
                  sa_add_date("$signup_time", time(NULL), 0, properties));
        // 用户是否购买过商品.
    ...
    // 3.1 记录用户提交订单事件.
    {
        ...
        SA_ASSERT(SA_OK ==
                  sa_add_string(
                      "$project", "MyOrder", strlen("MyOrder"), properties));
    ...
    // 4.3 不合法的事件名称.
    {
        SA_ASSERT(SA_INVALID_PARAMETER_ERROR ==
                  sa_track(login_id, "time", NULL, sa));
        SA_ASSERT(SA_INVALID_PARAMETER_ERROR ==
                  sa_track(login_id, "100vip", NULL, sa));
    }
