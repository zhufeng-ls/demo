...
template <class T> void RBTree<T>::preOrder() { preOrder(mRoot); }
...
template <class T> void RBTree<T>::inOrder() { inOrder(mRoot); }
...
template <class T> void RBTree<T>::postOrder() { postOrder(mRoot); }
...
template <class T> RBTNode<T> *RBTree<T>::search(T key) { search(mRoot, key); }
...
/*
 * 找结点(x)的后继结点。即，查找"红黑树中数据值大于该结点"的"最小结点"。
 * 当为右孩子，则依次向上，直到找到父节点为左子树的情形，此时叔节点一定是右子树中它的后继节点。
 * 若父节点不为做子树，则说明它没有后继节点，此时无需变动。
 */
...
template <class T> void RBTree<T>::destroy() { destroy(mRoot); }
